...
def ppg_findpeaks_v2(ppg_cleaned, sampling_rate=1000, method="elgendi", peakwindow=0.111, beatwindow=0.667, beatoffset=0.02, mindelay=0.3,show=False):

    method = method.lower()
    if method in ["elgendi"]:
        peaks = _ppg_findpeaks_elgendi(ppg_cleaned, sampling_rate,peakwindow, beatwindow, beatoffset, mindelay, show=show)
    else:
